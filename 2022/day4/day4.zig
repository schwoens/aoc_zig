...
fn part2(comptime len: usize, in: *const [len:0]u8) !u32 {
    ...
    while (line != null) : (line = lines.next()) {
        ...
        const range1_overlaps_range2 = (range1[0] >= range2[0] and range1[0] <= range2[1]) or (range1[1] >= range2[0] and range1[1] <= range2[1]);
        const range2_overlaps_range1 = (range2[0] >= range1[0] and range2[0] <= range1[1]) or (range2[1] >= range1[0] and range2[1] <= range1[1]);

        if (range1_overlaps_range2 or range2_overlaps_range1) {
            count += 1;
        }
    }
    return count;
